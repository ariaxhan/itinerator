...
`;
    ...
    try {
      const response = await fetch('/api/v1/projects/iterator-bf719/locations/us-west4/models/gemini-1.5-pro:predict', {
        method: 'POST',
        headers: {
   ...
   console.log('Document written with ID: ', docRef.id);
    ...
    if (error instanceof SyntaxError) {
      console.error('There was a syntax error in parsing JSON response.');
    } else if (error instanceof TypeError) {
      console.error('There was a type error somewhere.');
